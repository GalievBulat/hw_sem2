...
import com.kakadurf.hw_sem2.presentation.view.interfaces.ExtendedView
import com.kakadurf.hw_sem2.presentation.view_model.ExtendedScreenViewModel
import kotlinx.android.synthetic.main.fragment_extended_information.rv_quotes
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val viewModel = ViewModelProvider(this)
            .get(ExtendedScreenViewModel::class.java).apply {
                characterInfo.observe(
                    viewLifecycleOwner,
                    {
                        inflateCharacterData(it)
                    }
                )
                characterQuotes.observe(
                    viewLifecycleOwner,
                    {
